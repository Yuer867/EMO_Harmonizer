...
def events2dictionary(root, add_velocity=False, add_emotion=True, add_tempo=True,
    ...
    for file in event_files:
        events = pickle.load(open(os.path.join(event_path, file), 'rb'))[2]
        for event in events:
...
if __name__ == '__main__':
    ...
    if representation in ['absolute', 'transpose', 'transpose_rule', 'ablated']:
        relative = False
    elif representation == 'functional':
        relative = True
    else:
        raise ValueError("invalid representation {}, choose from ['absolute', 'transpose', 'transpose_rule', "
                         "'ablated', 'functional']".format(representation))

    events_dir = 'emopia_events/lead_sheet_chord11_{}'.format(representation)
    print(events_dir)
